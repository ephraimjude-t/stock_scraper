// src/components/LosersList.tsx
import React from "react";
import StockCard from "./StockCard";
import { useStockData } from "./hooks/useStockData";

const LosersList: React.FC = () => {
  const startEndpoint = "http://localhost:5000/api/start"; // Adjust as needed
  const dataEndpoint = "http://localhost:5000/api/top_losers";
  const { stocks, loading, error } = useStockData(dataEndpoint, startEndpoint);

  if (loading)
    return <div className="text-center py-4">Loading Top Losers…</div>;
  if (error)
    return (
